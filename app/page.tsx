...
export default function ConverterPage() {
  ...
  const fetchRef = useRef(false); // prevent double-fetch in dev StrictMode

  // ---- Fetch rate from our API route ----
  const fetchRate = useCallback(async () => {
    ...
    try {
      ...
      setRateData(data);
      // If live rate fetched successfully, clear any manual override
      if (data.rate !== null) setManualRate("");
    } catch {
      setRateData({
        rate: null,
        source: null,
        sourceUrl: "https://www.google.com/finance/quote/AUD-THB",
        fetchedAt: new Date().toISOString(),
        error: "เชื่อมต่อ API ไม่ได้ กรุณากรอกอัตราด้วยตนเอง",
      });
    } finally {
      setLoading(false);
      setFirstLoad(false);
  ...
  // Active rate: prefer live, fall back to manual input
  const activeRate =
      ...
      <div className="w-full max-w-md rounded-2xl border overflow-hidden shadow-2xl"
          ...
          <div className="flex items-start justify-between gap-3">
            <div>
              ...
              {firstLoad ? (
                ...
                <p className="text-xl font-semibold" style={{ color: "#e05d5d" }}>
                  ไม่พบอัตรา
                </p>
              )}
        ...
        {rateData?.error && (
          <div className="mx-6 mt-5 rounded-xl px-4 py-3 text-sm"
            ...
            <p className="font-semibold mb-2" style={{ color: "#e07070" }}>
              ⚠ {rateData.error}
            </p>
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                style={{
                  background: "var(--dark)",
                  borderColor: audValid ? "rgba(201,168,76,0.3)" : "rgba(224,93,93,0.5)",
                  color: "#f0ede6",
                }}
                placeholder="0.00"
              />
            ...
            {!audValid && aud !== "" && (
              <p className="text-xs mt-1" style={{ color: "#e07070" }}>
                กรุณากรอกจำนวนที่ถูกต้อง (≥ 0)
              </p>
            )}
          </div>
        ...
        <div className="px-6 py-5 fade-up">
          <p className="mono text-[10px] tracking-widest uppercase opacity-40 mb-4">
            ผลการคำนวณ
          </p>
          ...
          <div className="space-y-3">
            {/* Row: AUD input */}
            <ResultRow
              label="จำนวนเงินตั้งต้น"
              value={audValid ? `A$ ${fmt(audNum, 2)}` : "—"}
              accent={false}
            />
            {/* Row: Fee amount */}
            <ResultRow
              label={`Fee ${feeValid && feeNum > 0 ? fmt(feeNum, 2) + "%" : "0%"}`}
              value={
                audValid && feeValid && feeNum > 0
                  ? `− A$ ${fmt(audNum - netAud, 2)}`
                  : "A$ 0.00"
              }
              accent={false}
              muted
            ...
            {/* Row: Net AUD */}
            <ResultRow
              label="AUD หลังหัก fee"
              value={audValid && feeValid ? `A$ ${fmt(netAud, 2)}` : "—"}
              accent={false}
            />
            {/* Row: Exchange rate used */}
            <ResultRow
              label="อัตราที่ใช้"
              value={activeRate !== null ? `${fmt(activeRate, 4)} THB/AUD` : "ไม่มีอัตรา"}
              accent={false}
              muted
            />
          </div>
...
function ResultRow({
  ...
  return (
    <div className="flex items-center justify-between gap-3">
      <span
        className="text-sm"
        style={{ opacity: muted ? 0.45 : 0.65 }}>
        {label}
      </span>
      <span
        className="mono text-sm font-medium"
        style={{
          color: accent ? "var(--gold-light)" : "#f0ede6",
          opacity: muted ? 0.5 : 1,
        }}>
        {value}
      </span>
